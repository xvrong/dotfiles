...
def main():
    ...
    for si in sink_inputs:
        try:
            selections.append(
                si.proplist["application.name"] + ": " + si.proplist["media.name"]
            )
        except KeyError:
    ...
    sink_input_index = (
        -1 if sink_input_index == 0 else sink_inputs[sink_input_index - 1].index
    )
    ...
    sink_index, _ = rofi.select(
        "Select destination sink",
        [
            (
                s.description
                if s.description not in SINK_ALIASES
                else SINK_ALIASES[s.description]
            )
            for s in sinks
        ],
